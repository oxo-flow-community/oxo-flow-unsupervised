...
parser.add_argument("--clusterings", required=True)
parser.add_argument("--outputs", required=True, nargs="+")
args = parser.parse_args()

metadata_path = os.path.join(args.metadata)
clusterings_path = os.path.join(args.clusterings)
result_paths = args.outputs
...
for variable in metadata.columns:
    ...
    if (
        all([isinstance(i, (int, np.int64)) for i in unique_vals])
        and len(unique_vals) < 25
    ):
...
if not (set(clustering_results.index) == set(categorical_metadata.index)):
    if categorical_metadata.index.inferred_type == "string":
        categorical_metadata.index = [
            idx.replace("-", ".") for idx in categorical_metadata.index
        ]
...
for idx in indices:
    idx_dfs[idx] = pd.DataFrame(
        index=clustering_results.columns, columns=categorical_metadata.columns
    )
...
for clustering in clustering_results.columns:
    ...
    for metadata in categorical_metadata.columns:
        ...
        idx_dfs["AMI"].loc[clustering, metadata] = metrics.adjusted_mutual_info_score(
            categorical_metadata[metadata], clustering_results[clustering]
        )
        idx_dfs["ARI"].loc[clustering, metadata] = metrics.adjusted_rand_score(
            categorical_metadata[metadata], clustering_results[clustering]
        )
        idx_dfs["FMI"].loc[clustering, metadata] = metrics.fowlkes_mallows_score(
            categorical_metadata[metadata], clustering_results[clustering]
        )
