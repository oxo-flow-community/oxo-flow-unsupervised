...
def config_button_cat(fig, data, cat_var):
    ...
    # cm = plt.get_cmap('gist_ncar')
    colors = [cm(1.0 * i / (len(unique_vals) + 1)) for i in range(len(unique_vals) + 1)]
    # colors = [cm(1.*i/(len(unique_vals))) for i in range(len(unique_vals))]
    color_map = dict(zip(unique_vals, colors))

    tmp_idx = list(data.columns).index(cat_var) - dimensions
    data_colors = [
        color_map[datapoint[tmp_idx]] for datapoint in fig["data"][0]["customdata"]
    ]
    return [data_colors]
...
metadata_features_path = args.features

if args.clusterings != "":
    metadata_clusterings_path = args.clusterings
else:
    metadata_clusterings_path = ""

plot_path = args.out

dimensions = int(args.n_components)  # 2
point_size = (
    2 * float(args.size) if dimensions == 3 else 5 * float(args.size)
)  # 2
point_alpha = float(args.alpha)  # 1
                    ...
                    "args": [
                        {
                            "legendgroup": "",
                            "marker": {
                                "color": data_all[variable].to_numpy(),
                                "coloraxis": "coloraxis",
                                "symbol": "circle",
                                "size": point_size,
                            },
            ...
            "buttons": [
                {
                    ...
                    "args": [
                        {
                            "marker.color": config_button_cat(
                                fig_cat, data_all, variable
                            ),
