...
observations_distance_path <- get_arg(args, "--obs-dist")
features_distance_path <- get_arg(args, "--feat-dist")

# output
plot_path <- get_arg(args, "--out")

# parameters
samples_by_features <- as.integer(get_arg(args, "--samples-by-features"))
metric <- get_arg(args, "--metric")
cluster_method <- get_arg(args, "--hclust-method")
