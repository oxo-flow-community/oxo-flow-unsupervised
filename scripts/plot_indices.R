...
type <- get_arg(args, "--type") # "external" or "internal"

# index names: derived from the file names (upstream: named snakemake inputs)
if (type == "external") {
  indices <- sub("external_index_", "", sub("\\.csv$", "", basename(index_paths)))
} else {
  indices <- rep("ranked_internal_indices", length(index_paths))
}
...
for (idx in indices) {
  ...
  scores <- data.frame(fread(file.path(index_paths[which(indices == idx)][1]), header = TRUE), row.names = 1)
  ...
  heatmap_tmp <- ggplot(scores_plot, aes(x = metadata, y = clusterings, fill = value)) +
    ...
    {
      if (length(indices) == 1) {
        scale_x_discrete(labels = function(x) gsub(".*\\.", "", x), breaks = function(x) {
          x[x != "sep"]
        })
      }
    } +
