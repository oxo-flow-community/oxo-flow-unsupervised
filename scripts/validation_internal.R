...
clusterings_path <- get_arg(args, "--clusterings")
pca_path <- get_arg(args, "--pca")
pca_var_path <- get_arg(args, "--pca-var")

# output
result_path <- get_arg(args, "--out")

# parameters
internal_index <- as.character(get_arg(args, "--internal-index")) # "Silhouette"
sample_proportion <- as.numeric(get_arg(args, "--sample-proportion")) # 0.1
metadata_of_interest <- unlist(get_arg_list(args, "--metadata-of-interest"))
