...
parser.add_argument("--type", required=True)  # "observations" or "features"
parser.add_argument("--metric", required=True)
parser.add_argument("--n-observations", required=True)
parser.add_argument("--n-features", required=True)
args = parser.parse_args()

data_path = args.data
distance_matrix_path = args.out

samples_by_features = int(args.samples_by_features)
data_or_feature = args.type  # "observations" or "features"
metric = args.metric


def _coerce_num(value):
    # config values arrive as strings; upstream passed numbers
    if "." in value:
        return float(value)
    return int(value)


n_observations = _coerce_num(args.n_observations)
n_features = _coerce_num(args.n_features)

### load data

# check data orientation to fit: samples/observations x features
data = pd.read_csv(data_path, index_col=0)
if (samples_by_features == 0 and data_or_feature == "observations") or (
    samples_by_features == 1 and data_or_feature == "features"
):
    data = data.T
