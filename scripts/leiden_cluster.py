...
parser.add_argument("--partition-type", required=True)
parser.add_argument("--resolution", required=True)
parser.add_argument("--n-iterations", required=True)
args = parser.parse_args()

data_path = os.path.join(args.data)
graph_path = os.path.join(args.graph)
result_path = os.path.join(args.out)
...
partition_type = getattr(la, str(args.partition_type))
# Get the kwargs from the config dict
if str(args.partition_type) in [
    "RBConfigurationVertexPartition",
    "RBERVertexPartition",
    "CPMVertexPartition",
]:
    la_kwargs = {"resolution_parameter": float(args.resolution)}
else:
    la_kwargs = {}
...
pd.DataFrame({clustering_name: partition.membership}, index=data.index).to_csv(
    result_path, index=True
)
