...
#### port adaptation: read CLI arguments (upstream: snakemake object)
all_args <- commandArgs(trailingOnly = FALSE)
script_dir <- dirname(sub("^--file=", "", all_args[grepl("^--file=", all_args)][1]))
source(file.path(script_dir, "args.R"))
args <- commandArgs(trailingOnly = TRUE)

### configurations

# input
clustering_path <- get_arg(args, "--clusterings")
metadata_path <- get_arg(args, "--metadata")

# output
plot_path <- get_arg(args, "--out")

# parameters
content <- get_arg(args, "--content") # one of: metadata, features, default, custom
categorical_label_option <- as.character(get_arg(args, "--cat-label")) # "pure" or "majority"
numerical_aggregation_option <- as.character(get_arg(args, "--num-aggr")) # mean, median, max, min
custom_metadata <- get_arg_list(args, "--custom-metadata") # c("target")
count_filter <- as.numeric(get_arg(args, "--count-filter")) # 0
prop_filter <- as.numeric(get_arg(args, "--prop-filter")) # 0.1
layout <- as.character(get_arg(args, "--layout")) # "tree" or "sugiyama"
...
plot_clustree <- function(data, col) {
  clustree_plot <- clustree(
    ...
    {
      if (col == "X_") guides(color = "none")
    } + # remove legend in case of color being clusterings
    {
      if (is.factor(data[[col]]) & length(unique(data[[col]])) > 25) guides(color = "none")
    } + # remove legend in case of color being categorical with more than 25 categories
    ...
    {
      if (col == "X_") ggtitle(content) else ggtitle(col)
