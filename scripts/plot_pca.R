...
# inputs
data_path <- get_arg(args, "--data")
var_path <- get_arg(args, "--var")
axes_path <- get_arg(args, "--axes")
loadings_path <- get_arg(args, "--loadings")
metadata_path <- get_arg(args, "--metadata")

# outputs
diagnostics_path <- get_arg(args, "--out-variance")
pairs_path <- get_arg(args, "--out-pairs")
loadingsplot_path <- get_arg(args, "--out-loadings")
loadings_lollipop_plot_path <- get_arg(args, "--out-lollipop")

pairs_size <- as.numeric(get_arg(args, "--size")) / 10
pairs_alpha <- as.numeric(get_arg(args, "--alpha")) / 2
metadata_col <- c(get_arg_list(args, "--metadata-of-interest"))[1]
...
if (nrow(data) > 0) {
  ...
  pairs_plot <- ggplot() +
    annotate("text", x = 0.5, y = 0.5, label = "No group with more than 2 members in the data.") +
    theme_void()
  n_dim <- 5
}
